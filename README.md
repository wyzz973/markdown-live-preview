...
| 状态 | 侧栏 | 文档来源 | 保存到 |
| --- | --- | --- | --- |
| 什么都没开 | 当前草稿的结构 | 草稿缓冲区 | `localStorage`，防抖 300ms |
| 打开了单个文件 | 这个文件的结构 | 磁盘上的那个文件 | 原文件，防抖 700ms |
| 打开了文件夹 | 文件树 + 当前文件的结构 | 选中的那个文件 | 原文件，防抖 700ms |

单个文件不是"只有一个文件的文件夹"：它在数据结构上和文件夹里的条目完全一样（同一套读写路径），区别只在于外面有没有一层可浏览、可检索的库。
...
src/modules/
  ...
  rail.js                  侧栏：文件树 + 标题树
  toolbar.js               语法工具栏
  workspace.js             草稿 / 单文件 / 文件夹三种状态与保存路径
...
在 Firefox 和 Safari 里：文件夹相关的入口自动隐藏，而不是留下点不动的按钮；打开单个文件和拖放仍然可用，但拿到的只有字节而没有句柄，所以文档是以**副本**载入草稿缓冲区的，改动不会写回磁盘——提示语会直说这一点，而不是假装保存成功。
